...
const int ROCKET_MASS = 90; //Mass of rocket in Kg
...
void reset_fins(){ //Set fins to original position
  setFins_ALL(0);
  delay(400);
}

void led_blink(int reps,int delay_time){ //Blink LED for notification.
...
void setFins_ALL(int fin_position){ //Set all fins to certain position. Used in roll and testing operations
  if(fin_position > 180){
    fin_position -= 180;
  }
...
void testFins(){ //Function for ground testing fins. 
  setFins_ALL(120);
  delay(800);
  setFins_ALL(60);
  delay(400);
...
void printTimeStamp(){ //For printing time stamps in log entries
  timeNow = millis();
...
void calculateFinMovements(boolean stabilise, int x, int y, int z){  
  getOrientation();
  x -= ax;
  y -= ay;
  z -= az;
...
void executeMovement(){ //Placeholder for executing exe_ variables. 
  Serial.println(ax);
  Serial.println(ay);
  Serial.println(az);
}
